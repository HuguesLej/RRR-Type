...
namespace comp
{
    ...
    struct Controllable
    {
        ...
        Controllable(Keys left, Keys right, Keys up, Keys down, Keys jump, std::string localAddress, uint16_t localPort)
            : left({left, false}), right({right, false}), up({up, false}), down({down, false}), jump({jump, false}), localAddress(localAddress), localPort(localPort) {}
